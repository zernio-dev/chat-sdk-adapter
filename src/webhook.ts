 ...
 * The signature is sent in the X-Late-Signature header as a hex digest.
 ...
 * @param signature - The hex digest from the X-Late-Signature header
...
export function extractWebhookHeaders(request: Request): {
  ...
  return {
    ...
    event: request.headers.get("x-late-event"),
  };
}
